...
namespace WebApi2Book.Data
{
    public static class ResultsPagingUtility
    {
        ...
        public static int CalculatePageSize(int requestedValue, int maxValue)
        {
            if (requestedValue < 1)
                throw new ArgumentOutOfRangeException(
                    "requestedValue", requestedValue, ValueLessThanOneErrorMessage);
            if (maxValue < 1)
                throw new ArgumentOutOfRangeException(
                    "maxValue", maxValue, ValueLessThanOneErrorMessage);
        ...
        public static int CalculateStartIndex(int pageNumber, int pageSize)
        {
            if (pageNumber < 1)
                throw new ArgumentOutOfRangeException(
                    Constants.CommonParameterNames.PageNumber, pageNumber,
                    ValueLessThanOneErrorMessage);
            if (pageSize < 1)
                throw new ArgumentOutOfRangeException(
                    Constants.CommonParameterNames.PageSize, pageSize,
                    ValueLessThanOneErrorMessage);
        ...
        {
            if (totalItemCount < 0)
                throw new ArgumentOutOfRangeException("totalItemCount", totalItemCount,
                    ValueLessThanZeroErrorMessage);
            if (pageSize < 1)
                throw new ArgumentOutOfRangeException(Constants.CommonParameterNames.PageSize,
                    pageSize, ValueLessThanOneErrorMessage);
